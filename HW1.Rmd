...
$$\sum_{i,x_i=1}^n x_i=n_1$$
$n_0=n-n_1$

$\bar{x}=\frac{\sum_{i,x_1=1}^n x_i}{n}=\frac{n_1}{n_1+n_0}$
...
=\frac{\displaystyle\sum_{i=1}^n (x_i-\bar{x})y_i}{\displaystyle\sum_{i=1}(x_i-\bar{x})}\\
&=\frac{\displaystyle\sum_{i=1}^n (x_i-\bar{x})(\beta_0+\beta_1x_i+u_i)}{\displaystyle\sum_{i=1}(x_i-\bar{x})}\\
...
+\frac{\displaystyle\sum_{i=1}^n(x_i-\bar{x})u_i}{\displaystyle\sum_{i=1}^n(x_i-\bar{x})^2}\\
&=0+\frac{\displaystyle\beta_1}{\sum_{i=1}(x_i-\bar{x})^2}\displaystyle\sum_{i=1}(x_i-\bar{x})^2
+\frac{\displaystyle\sum_{i=1}^n(x_i-\bar{x})u_i}{\displaystyle\sum_{i=1}^n(x_i-\bar{x})^2}\\
&=\beta_1+\frac{\sum_{i=1}^n(x_i-\bar{x})u_i}{\displaystyle\sum_{i=1}^n(x_i-\bar{x})^2}
\end{split}
\end{equation*}$$
...
\begin{split}
E(\hat{\beta}_1)
=E(\beta_1+\frac{\displaystyle\sum_{i=1}^n(x_i-\bar{x})u_i}{\displaystyle\sum_{i=1}^n(x_i-\bar{x})^2})=\beta_1+\frac{\displaystyle\sum_{i=1}^n(x_i-\bar{x})E(u_i)}{\displaystyle\sum_{i=1}^n(x_i-\bar{x})^2}
=\beta_1
\end{split}
\end{equation*}$$
...
\begin{split}
Var(\hat{\beta}_1|X)
&=E(\beta_1+\frac{\displaystyle\sum_{i=1}^n(x_i-\bar{x})u_i}{\displaystyle\sum_{i=1}^n(x_i-\bar{x})^2})=\beta_1+\frac{\displaystyle\sum_{i=1}^n(x_i-\bar{x})Var(u_i)}{\displaystyle\sum_{i=1}^n(x_i-\bar{x})^2}\\
...
$$\begin{equation*}
\begin{split}
E(\epsilon_i^2|x_i=0)=\sigma_0^2
\end{split}
\end{equation*}$$

$$\begin{equation*}
\begin{split}
E(\epsilon_i^2|x_i=1)=\sigma_1^2
\end{split}
\end{equation*}$$
...
&=E(\beta_0^2+2\beta_0\epsilon_i+\epsilon_i^2|X)-\beta_0^2\\
&=\beta_0^2+\sigma_0^2-\beta_0^2\\
&=\sigma_0^2
\end{split}
\end{equation*}$$
...
params:
  name: ["余亞諺","黃智遠","陳廷瑋","顏煜名"]
  id: [710761114,710761111,710761119,410573099]
  group: "team D"
  url: ""
---
請依狀況更改上面的name,id及group（分別代表組員姓名，學號及組號），但請勿更改下面三行的設定。  
...
Ans:
  ...
  1. 比較「第一次經原通過，第二次個經被當」與「第一次經原被當、第二次個經也被當」的人數比例(這兩群人最好有相近的經原PR，以確保有相同的立足點)。透過這個比較，我們得以知道擋修制度究竟是讓經原被當的學生改過自新、奮發圖強?　還是學生用同樣造成他經原被當的學習模式去面對個經?
...
>>>>>>> 35155869fbd7d37785aa4509a4757368b666cce9
